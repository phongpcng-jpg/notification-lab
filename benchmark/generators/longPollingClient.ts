...
export class LongPollingClient implements SimulatedClient {
  ...
  private after=0; private stopped=true; private abortController:AbortController|null=null;
  ...
  async disconnect():Promise<void>{this.stopped=true;this.abortController?.abort();}
  private async loop():Promise<void>{
    if(this.stopped)return;
    const controller=new AbortController();this.abortController=controller;
    try{
      const res=await fetch(`${apiBaseUrl()}/notifications/long-poll?userId=${this.userId}&after=${this.after}`,{signal:controller.signal});
      ...
      for(const n of body.notifications)this.events.push({notificationId:n.id,receivedAtMonoMs,serverCreatedAtMs:n.created_at*1000,serverSentAtMs:body.serverSentAtMs});
      this.after=body.nextAfter;
      if(!this.stopped)void this.loop();
    }catch(err){if(controller.signal.aborted)return;this.errorCount++;if(!this.stopped)setTimeout(()=>void this.loop(),1000);}
  }
}
